...
func run() -> void:
	...
	for door_x in [-2.8, 0.0, 2.8]:
		game.player.position = Vector3(door_x, 0.05, -20.05)
		await settle()
		aim_point(Vector3(door_x, 3.5, -20.35))
		check(sim.can_leave_room(), "visible door accepts close contact and upward aim at x=%s" % door_x)
	sim.yaw = PI
	check(not sim.can_leave_room(), "facing away from doorway cannot interact")
	game.player.position = Vector3(0, 0.05, -16)
	sim.submit_input(Vector2.ZERO, false, Vector2.ZERO, true)
	await ticks(45)
	check(sim.phase == "training" and sim.gate_hold == 0, "holding F too far from door cannot leave")
	sim.submit_input(Vector2(0, -1), false, Vector2.ZERO)
	await ticks(65)
	check(sim.can_leave_room() and game.player.position.z < -19, "walking up to visible door keeps interaction available")
	sim.submit_input(Vector2.ZERO, false, Vector2.ZERO, true)
	await ticks(20)
	check(sim.gate_hold > 0 and sim.phase == "training", "F hold displays progress before travel")
	await capture("tutorial-door-hold")
	sim.submit_input(Vector2.ZERO, false, Vector2.ZERO)
	await ticks(1)
	check(sim.gate_hold == 0, "releasing F resets door progress")
	sim.submit_input(Vector2.ZERO, false, Vector2.ZERO, true)
	await ticks(40)
	check(sim.phase == "travel", "holding F at the actual door starts travel")
	await ticks(80)
